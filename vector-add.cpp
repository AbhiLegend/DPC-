...
//************************************
// Compute vector addition in DPC++ on device: multiplication of the data is returned in
// 4th parameter "mul_parallel"
//************************************
void VectorMulInDPCPP(const IntArray &addend_1, const IntArray &addend_2, const IntArray &addend_3,
                      IntArray &multiplication_parallel) {
  ...
  buffer<int, 1> addend_3_buf(addend_3.data(), num_items);
  //multiplication
  buffer<int, 1> mul_buf(multiplication_parallel.data(), num_items);
  ...
  q.submit([&](handler &h) {
	...
	auto addend_3_accessor = addend_3_buf.get_access<dp_read>(h);

    // the sum_accessor is used to store (with write permision) the sum data
    auto mul_accessor = mul_buf.get_access<dp_write>(h);
    ...
    h.parallel_for(num_items, [=](id<1> i) {
      mul_accessor[i] = addend_1_accessor[i] * addend_2_accessor[i] * addend_3_accessor[i];
    });
  });
...
int main() {
  ...
  // data
  IntArray addend_1, addend_2, addend_3, mul_scalar, mul_parallel;

  // Initialize input arrays with values from 0 to array_size-1
  initialize_array(addend_1);
  initialize_array(addend_2);
  initialize_array(addend_3);

  // Compute vector multiplication in DPC++
  VectorMulInDPCPP(addend_1, addend_2, addend_3, mul_parallel);

  // Computes the multiplication of three arrays in scalar for validation
  for (size_t i = 0; i < mul_scalar.size(); i++)
    mul_scalar[i] = addend_1[i] * addend_2[i] * addend_3[i];

  // Verify that the three product arrays are equal
  for (size_t i = 0; i < mul_parallel.size(); i++) {
    if (mul_parallel[i] != mul_scalar[i]) {
      std::cout << "fail" << std::endl;
      return -1;
    }
  }
  std::cout << "successout" << std::endl;
